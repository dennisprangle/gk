...
#'
#' Finite difference stochastic approximation (FDSA) inference for the 5 parameter g-and-k or g-and-h distribution
#'
...
#' @param theta0 Vector of initial value for 5 parameters.
...
#' out = fdsa2(x, N=100, theta0=c(mean(x),sd(x),0,0,0), theta_min=c(-5,1E-5,-5,0,0), theta_max=c(5,5,5,5,5))
#' @export
fdsa2 = function(x, N, model=c("gk", "gh"), logB=FALSE, theta0, batch_size=100, alpha=1, gamma=0.49, a0=1, c0=NULL, A=100,
    theta_min=c(-Inf,ifelse(logB, -Inf, 1E-5),-Inf,0,0), theta_max=c(Inf,Inf,Inf,Inf,Inf),
    silent=FALSE, plotEvery=100) {
    if (!silent) { oldask = par(ask=FALSE) } ##Don't ask before progress plots
    cnames = c("A", ifelse(logB, "log B", "B"), "g", ifelse(model[1]=="gk", "k+", "h+"), ifelse(model[1]=="gk", "k-", "h-"), "estimated log likelihood")
    if (model[1] == "gk") {
        if (logB) {
            get_log_densities = function(x, theta) dgk(batch, theta[1], exp(theta[2]), theta[3], ifelse(batch>=theta[1], theta[4], theta[5]), log=TRUE)
        } else {
            get_log_densities = function(x, theta) dgk(batch, theta[1], theta[2], theta[3], ifelse(batch>=theta[1], theta[4], theta[5]), log=TRUE)
        }
    } else {
        if (logB) {
            get_log_densities = function(x, theta) dgh(batch, theta[1], exp(theta[2]), theta[3], ifelse(batch>=theta[1], theta[4], theta[5]), log=TRUE)
        } else {
            get_log_densities = function(x, theta) dgh(batch, theta[1], theta[2], theta[3], ifelse(batch>=theta[1], theta[4], theta[5]), log=TRUE)
        }
    }
    nobs = length(x)
    ...
    theta = theta0
    estimates = matrix(nrow=N+1, ncol=6)
    ...
    for (t in seq(0,N-1)) {
        ...
        estimates[t+2,1:5] = theta
        estimates[t+2,6] = log(2) - logSumExp(hatL1, hatL2) ##Log of mean likelihood estimate
